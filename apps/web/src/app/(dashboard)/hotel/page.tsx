...
const STATUS_CONFIG: Record<RoomStatus, { label: string; color: string; dot: string }> = {
  available: { label: 'Available', color: 'bg-emerald-500/15 border-emerald-500/40 text-emerald-600 dark:text-emerald-300', dot: 'bg-emerald-500' },
  occupied: { label: 'Occupied', color: 'bg-blue-500/15    border-blue-500/40    text-blue-300', dot: 'bg-blue-500' },
  reserved: { label: 'Reserved', color: 'bg-amber-100 dark:bg-amber-500/15   border-amber-400 dark:border-amber-500/40   text-amber-600 dark:text-amber-300', dot: 'bg-amber-500' },
...
function RoomDetail({ room, onClose }: { room: RoomSummary; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 w-full max-w-xs space-y-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between">
          <div>
            <div className="text-xl font-bold text-slate-900 dark:text-white">Room {room.roomNumber}</div>
            <div className="text-sm text-slate-900 dark:text-slate-400">{room.roomType?.name} · Floor {room.floor}</div>
          </div>
        ...
        <div className="text-sm text-slate-900 dark:text-slate-400">
          Base rate: <span className="text-slate-900 dark:text-white font-semibold">₹{Number(room.roomType?.baseRate ?? 0).toLocaleString('en-IN')}/night</span>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={onClose} className="px-3 py-2 text-xs bg-slate-50 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-lg transition-colors">
            Close
          </button>
        </div>
      </div>
      ...
      <div className="p-6 space-y-4 animate-pulse">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-24 rounded-xl bg-slate-50 dark:bg-slate-800" />
          ))}
        </div>

        <div className="h-64 rounded-xl bg-slate-50 dark:bg-slate-800" />

        <div className="grid md:grid-cols-2 gap-4">
          <div className="h-40 rounded-xl bg-slate-50 dark:bg-slate-800" />
          <div className="h-40 rounded-xl bg-slate-50 dark:bg-slate-800" />
        </div>
      </div>
    );
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-semibold text-slate-900 dark:text-white">Hotel</h1>
            <p className="text-xs text-slate-900 dark:text-slate-500">Front desk overview</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => refetch()}
            className="p-1.5 rounded-lg bg-slate-50 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-slate-400 transition-colors"
          >
            <RefreshCw size={13} />
          </button>
          ...
          <Link
            ...
            className="text-xs flex items-center gap-1.5 px-3 py-2 rounded-lg
               bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700
               hover:bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-white transition-colors"
          >
            <BedDouble size={13} />
            Add Room
          </Link>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            { label: 'Total Rooms', value: stats.totalRooms, color: 'text-slate-600 dark:text-slate-300' },
            { label: 'Occupancy', value: `${stats.occupancyPct}%`, color: 'text-blue-400' },
            { label: 'In House', value: stats.inHouse, color: 'text-blue-400' },
            { label: 'Arrivals Today', value: stats.arrivalsToday, color: 'text-amber-600 dark:text-amber-400' },
            { label: 'Departures', value: stats.departuresToday, color: 'text-violet-400' },
            { label: 'Available', value: stats.roomsByStatus?.available ?? 0, color: 'text-emerald-600 dark:text-emerald-400' },
          ].map(({ label, value, color }) => (
            <div key={label} className="card text-center">
              <div className={cn('text-2xl font-bold tabular-nums', color)}>{value}</div>
              <div className="text-[11px] text-slate-900 dark:text-slate-500 mt-0.5">{label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          {(Object.entries(STATUS_CONFIG) as [RoomStatus, typeof STATUS_CONFIG[RoomStatus]][]).map(([status, cfg]) => (
            <div key={status} className="flex items-center gap-1.5 text-xs text-slate-900 dark:text-slate-400">
        ...
        {rooms.length === 0 ? (
          <div className="card text-center py-12 space-y-3">
            ...
            <p className="text-slate-900 dark:text-slate-500 text-sm">No rooms configured yet.</p>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="card space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Link href="/hotel/reservations?status=confirmed" className="text-xs text-slate-900 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300">
                View all
              </Link>
            </div>
            {arrivals.length === 0 ? (
              ...
              <div className="divide-y divide-slate-200 dark:divide-slate-800/60">
                {arrivals.map((r) => (
                  <div key={r.id} className="py-2 flex items-center justify-between gap-3">
                    ...
                    <span className="text-xs bg-amber-100 dark:bg-amber-500/15 text-amber-600 dark:text-amber-400 px-2 py-0.5 rounded-md flex-shrink-0">
                      {r.room?.roomNumber}
                    </span>
                  </div>
                ))}
          ...
          <div className="card space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Link href="/hotel/reservations?status=checked_in" className="text-xs text-slate-900 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300">
                View all
              </Link>
            </div>
            {departures.length === 0 ? (
