...
export default function HotelDashboardPage() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-slate-50 dark:bg-slate-950 text-slate-700 dark:text-slate-200">
      {/* Fixed Header */}
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-5 px-4 lg:px-8 pt-4">
        <div>
        ...
        {isLoading ? (
          <div className="space-y-6 animate-pulse">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[...Array(4)].map((_, i) => <div key={i} className="bg-slate-200 dark:bg-slate-800 rounded-2xl h-24" />)}
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="bg-slate-200 dark:bg-slate-800 rounded-2xl h-[300px] lg:col-span-2" />
              <div className="bg-slate-200 dark:bg-slate-800 rounded-2xl h-[300px]" />
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 pt-10">
              {[...Array(5)].map((_, i) => <div key={i} className="bg-slate-200 dark:bg-slate-800 rounded-2xl h-28" />)}
            </div>
          </div>
        ) : (
          <>
