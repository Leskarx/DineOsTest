...
export default function HotelReportsPage() {
    ...
    // ── Paginated data (only when data exceeds LIMIT) ───────────────────────────
    const paginatedRevenue = revenueReport?.length > LIMIT
        ? revenueReport.slice((revenuePage - 1) * LIMIT, revenuePage * LIMIT)
        : revenueReport || [];
    const revenueTotalPages = Math.ceil((revenueReport?.length || 0) / LIMIT);

    const paginatedBookings = bookingsReport?.length > LIMIT
        ? bookingsReport.slice((bookingsPage - 1) * LIMIT, bookingsPage * LIMIT)
        : bookingsReport || [];
    const bookingsTotalPages = Math.ceil((bookingsReport?.length || 0) / LIMIT);

    const paginatedRooms = roomsReport?.length > LIMIT
        ? roomsReport.slice((roomsPage - 1) * LIMIT, roomsPage * LIMIT)
        : roomsReport || [];
    const roomsTotalPages = Math.ceil((roomsReport?.length || 0) / LIMIT);

    const paginatedGst = gstReport?.length > LIMIT
        ? gstReport.slice((gstPage - 1) * LIMIT, gstPage * LIMIT)
        : gstReport || [];
    const gstTotalPages = Math.ceil((gstReport?.length || 0) / LIMIT);

    const paginatedFrontdesk = frontdeskReport?.length > LIMIT
        ? frontdeskReport.slice((frontdeskPage - 1) * LIMIT, frontdeskPage * LIMIT)
        : frontdeskReport || [];
    const frontdeskTotalPages = Math.ceil((frontdeskReport?.length || 0) / LIMIT);
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden bg-slate-50 dark:bg-slate-950">
            ...
            <div className="flex-shrink-0 p-6 space-y-6 border-b border-slate-200 dark:border-slate-800">
                {/* ── Header ─────────────────────────────────────────────────────────── */}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                {!loading && tab === 'revenue' && (
                    <div className="space-y-4">
                        ...
                        <div className={cn(
                            ...
                            revenueReport?.length > LIMIT && "h-[500px]"
                        )}>
                            <div className={cn(
                                revenueReport?.length > LIMIT && "flex-1 overflow-auto"
                            )}>
                            ...
                            {revenueReport?.length > LIMIT && (
                                <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-slate-700">
                                    <span className="text-xs text-slate-500">
                                        Showing {(revenuePage - 1) * LIMIT + 1} - {Math.min(revenuePage * LIMIT, revenueReport?.length || 0)} of {revenueReport?.length || 0}
                                    </span>
                                    <div className="flex gap-2 items-center">
                                        ...
                                        <span className="text-sm">{revenuePage} / {revenueTotalPages}</span>
                                        <button
                ...
                {!loading && tab === 'bookings' && (
                    <div className="space-y-4">
                        ...
                        <div className={cn(
                            "card p-0 overflow-hidden flex flex-col",
                            bookingsReport?.length > LIMIT && "h-[500px]"
                        )}>
                            <div className={cn(
                                bookingsReport?.length > LIMIT && "flex-1 overflow-auto"
                            )}>
                                <table className="w-full text-sm">
                                    <thead className={cn(
                                        "sticky top-0 z-10 bg-slate-100/50 dark:bg-slate-800/50",
                            ...
                            {bookingsReport?.length > LIMIT && (
                                <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-slate-700">
                                    <span className="text-xs text-slate-500">
                                        Showing {(bookingsPage - 1) * LIMIT + 1} - {Math.min(bookingsPage * LIMIT, bookingsReport?.length || 0)} of {bookingsReport?.length || 0}
                                    </span>
                                    <div className="flex gap-2 items-center">
                                        <button
                                        ...
                                        <span className="text-sm">{bookingsPage} / {bookingsTotalPages}</span>
                                        <button
                ...
                {!loading && tab === 'rooms' && (
                    <div className="space-y-4">
                        ...
                        <div className={cn(
                            "card p-0 overflow-hidden flex flex-col",
                            roomsReport?.length > LIMIT && "h-[500px]"
                        )}>
                            <div className={cn(
                                roomsReport?.length > LIMIT && "flex-1 overflow-auto"
                            )}>
                                <table className="w-full text-sm">
                                    <thead className={cn(
                                        "sticky top-0 z-10 bg-slate-100/50 dark:bg-slate-800/50",
                            ...
                            {roomsReport?.length > LIMIT && (
                                <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-slate-700">
                                    <span className="text-xs text-slate-500">
                                        Showing {(roomsPage - 1) * LIMIT + 1} - {Math.min(roomsPage * LIMIT, roomsReport?.length || 0)} of {roomsReport?.length || 0}
                                    </span>
                                    <div className="flex gap-2 items-center">
                                        <button
                                        ...
                                        <span className="text-sm">{roomsPage} / {roomsTotalPages}</span>
                                        <button
                ...
                {/* ════════ PAYMENTS TAB (No pagination needed) ════════ */}
                ...
                {!loading && tab === 'gst' && (
                    <div className="space-y-4">
                        <div className={cn(
                            "card p-0 overflow-hidden flex flex-col",
                            gstReport?.length > LIMIT && "h-[500px]"
                        )}>
                            <div className={cn(
                                gstReport?.length > LIMIT && "flex-1 overflow-auto"
                            )}>
                                <table className="w-full text-sm">
                                    <thead className={cn(
                                        "sticky top-0 z-10 bg-slate-100/50 dark:bg-slate-800/50",
                                    ...
                                    {gstReport?.length > 0 && (
                                        <tfoot className="bg-slate-50 dark:bg-slate-800/80 sticky bottom-0">
                                            <tr>
                            ...
                            {gstReport?.length > LIMIT && (
                                <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-slate-700">
                                    <span className="text-xs text-slate-500">
                                        Showing {(gstPage - 1) * LIMIT + 1} - {Math.min(gstPage * LIMIT, gstReport?.length || 0)} of {gstReport?.length || 0}
                                    </span>
                                    <div className="flex gap-2 items-center">
                                        <button
                                            disabled={gstPage === 1}
                                            onClick={() => setGstPage(p => p - 1)}
                                            className="btn-secondary px-3 py-1.5 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            <ChevronLeft size={16} />
                                        </button>
                                        <span className="text-sm">{gstPage} / {gstTotalPages}</span>
                                        <button
                ...
                {!loading && tab === 'frontdesk' && (
                    <div className="space-y-4">
                        ...
                        <div className={cn(
                            "card p-0 overflow-hidden flex flex-col",
                            frontdeskReport?.length > LIMIT && "h-[500px]"
                        )}>
                            <div className={cn(
                                frontdeskReport?.length > LIMIT && "flex-1 overflow-auto"
                            )}>
                                <table className="w-full text-sm">
                                    <thead className={cn(
                                        "sticky top-0 z-10 bg-slate-100/50 dark:bg-slate-800/50",
                                    ...
                                    <tbody>
                                        {paginatedFrontdesk.map((staff: any, i: number) => (
                            ...
                            {frontdeskReport?.length > LIMIT && (
                                <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-slate-700">
                                    <span className="text-xs text-slate-500">
                                        Showing {(frontdeskPage - 1) * LIMIT + 1} - {Math.min(frontdeskPage * LIMIT, frontdeskReport?.length || 0)} of {frontdeskReport?.length || 0}
                                    </span>
                                    <div className="flex gap-2 items-center">
                                        <button
                                            disabled={frontdeskPage === 1}
                                            onClick={() => setFrontdeskPage(p => p - 1)}
                                            className="btn-secondary px-3 py-1.5 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            <ChevronLeft size={16} />
                                        </button>
                                        <span className="text-sm">{frontdeskPage} / {frontdeskTotalPages}</span>
                                        <button
                                            disabled={frontdeskPage === frontdeskTotalPages}
                                            onClick={() => setFrontdeskPage(p => p + 1)}
                                            className="btn-secondary px-3 py-1.5 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            <ChevronRight size={16} />
