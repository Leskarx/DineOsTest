    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-semibold text-white">Hotel Billing</h1>
            <p className="text-xs text-slate-500">Guest invoices generated at checkout</p>
          </div>
        </div>
        <button
          onClick={() => refetch()}
          disabled={isFetching}
          className="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-400 disabled:opacity-50 transition-colors"
        >
          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0 z-10 bg-slate-900 border-b border-slate-800">
              <tr>
                ...
                <th className="text-right px-4 py-3 font-medium text-slate-400">Paid</th>
                <th className="text-right px-4 py-3 font-medium text-slate-400">Balance</th>
              ...
              {filteredBills.map((bill) => {
                ...
                return (
                  <tr key={bill.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="px-4 py-3 font-mono font-medium text-white">{bill.billNumber}</td>
                    <td className="px-4 py-3 text-slate-300">{bill.customerName ?? '—'}</td>
                    <td className="px-4 py-3 text-slate-400">{fmtDate(bill.createdAt)}</td>
                    <td className="px-4 py-3 text-right text-slate-200">{formatCurrency(Number(bill.grandTotal))}</td>
                    <td className="px-4 py-3 text-right text-emerald-400">{formatCurrency(Number(bill.paidAmount))}</td>
                    <td className="px-4 py-3 text-right">
                      {balance > 0.01
                        ? <span className="text-red-400">{formatCurrency(balance)}</span>
                        : <span className="text-slate-600">—</span>}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={cn('px-2 py-0.5 rounded-md text-[10px] font-semibold uppercase tracking-wide', {
                        'bg-emerald-500/15 text-emerald-400': bill.status === 'paid',
                        'bg-red-500/15 text-red-400': bill.status === 'void',
                        'bg-amber-500/15 text-amber-400': bill.status === 'issued' || bill.status === 'draft',
                      })}>
                        {bill.status}
                      </span>
                    </td>
