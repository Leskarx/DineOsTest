...
export default function BillingPage() {
  ...
  const { branchName } = useAuthStore();  // ← get branch name from store
  ...
  /* ── Build print args — uses real branch name + pre-tax item amounts ── */
  const buildPrintArgs = (bd: any) => {
    const orderType  = resolveOrderType(bd);
    const isDelivery = orderType === 'delivery';
    const isCOD      = detectCod(bd);
    return {
      restaurantName:  branchName || 'Restaurant',   // ← real branch name
      ...
      items: (bd.orderItems ?? []).map((i: any) => ({
        ...
        // ✅ Pre-tax amount — unitPrice × qty (NOT lineTotal which includes GST)
        amount: Number(i.unitPrice) * Math.round(Number(i.quantity)),
      })),
        ...
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 space-y-3 flex-shrink-0">
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2 text-sm">
              <input type="date" className="input py-1.5" value={from} onChange={(e) => handleFromChange(e.target.value)} />
              <span className="text-slate-500">to</span>
              <input type="date" className="input py-1.5" value={to}   onChange={(e) => handleToChange(e.target.value)} />
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-300 dark:border-slate-700 w-full max-w-sm p-6 space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={() => { setEmailModal(null); setEmailInput(''); }} className="btn-secondary flex-1">
                Cancel
              </button>
              <button
                    ...
                    {(billDetail.orderItems ?? []).map((item: any) => (
                      <div key={item.id} className="flex justify-between text-sm">
                        <span className="text-slate-600 dark:text-slate-300">
                          {Math.round(Number(item.quantity))}× {item.name}
                        </span>
                        <span className="text-slate-900 dark:text-white font-medium">
                          ₹{(Number(item.unitPrice) * Math.round(Number(item.quantity))).toFixed(2)}
                        </span>
                      </div>
                    ))}
                  </div>
