...
const ORDER_STATUS_COLOR: Record<string, string> = {
  ...
  ready: 'text-emerald-400 bg-emerald-900/20',
...
export default function DashboardPage() {
  ...
  const stats = [
    { label: "Today's Sales", value: `₹${Number(summary?.todaySales || 0).toLocaleString('en-IN')}`, icon: IndianRupee, color: 'text-amber-400' },
    ...
    { label: 'Low Stock Alerts', value: summary?.lowStockAlerts || 0, icon: AlertTriangle, color: 'text-red-400' },
  ];
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400">{dayjs().format('dddd, D MMMM YYYY')}</p>
        </div>

        {/* Shift Status Widget */}
        {shift ? (
          <div className="flex items-center gap-3 bg-emerald-900/20 border border-emerald-700 rounded-xl px-4 py-2.5">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse flex-shrink-0" />
            <div>
              <div className="flex items-center gap-2">
                <ChefHat size={14} className="text-emerald-400" />
                <span className="text-emerald-300 font-medium text-sm">Shift Open</span>
              </div>
            ...
            <button
              ...
              className="ml-2 flex items-center gap-1.5 text-xs text-red-400 hover:text-red-300 bg-red-900/20 hover:bg-red-900/40 border border-red-800 rounded-lg px-2.5 py-1.5 transition-all"
            >
          ...
          <button
            ...
            className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-amber-600 rounded-xl px-4 py-2.5 text-sm transition-all group"
          >
            <span className="w-2 h-2 rounded-full bg-slate-500 group-hover:bg-amber-500 transition-colors" />
            <span className="text-slate-400 group-hover:text-white">No Active Shift</span>
            <Play size={13} className="text-amber-400 ml-1" />
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="card lg:col-span-2">
          <h2 className="text-sm font-semibold text-slate-300 mb-4">Hourly Sales — Today</h2>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={hourly || []} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
              <XAxis dataKey="hour" tick={{ fill: '#94a3b8', fontSize: 11 }} tickFormatter={(h) => `${h}:00`} />
              ...
              <Bar dataKey="revenue" radius={[4, 4, 0, 0]}>
                {(hourly || []).map((_: any, i: number) => (
                  <Cell key={i} fill={i === dayjs().hour() ? '#f59e0b' : '#334155'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className="card flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-slate-300">7-Day Revenue</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-slate-300">Order Statistics — Today</h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-800/60 rounded-xl p-3 flex items-center gap-3">
              ...
              <div>
                <div className="text-lg font-bold text-white">{summary?.orderStats?.successful ?? 0}</div>
            ...
            <div className="bg-slate-800/60 rounded-xl p-3 flex items-center gap-3">
              ...
              <div>
                <div className="text-lg font-bold text-white">{summary?.orderStats?.cancelled ?? 0}</div>
            ...
            <div className="bg-slate-800/60 rounded-xl p-3 flex items-center gap-3">
              ...
              <div>
                <div className="text-lg font-bold text-white">{summary?.orderStats?.complimentary ?? 0}</div>
                <div className="text-xs text-slate-500">Complimentary</div>
              </div>
            </div>
            <div className="bg-slate-800/60 rounded-xl p-3 flex items-center gap-3">
              ...
              <div>
                <div className="text-lg font-bold text-white">{summary?.orderStats?.returns ?? 0}</div>
                <div className="text-xs text-slate-500">Returns</div>
              </div>
            </div>
          </div>
          {/* Avg turnaround */}
          <div className="mt-3 flex items-center gap-2 bg-slate-800/40 rounded-xl px-4 py-3">
            <Timer size={14} className="text-purple-400 flex-shrink-0" />
            <span className="text-xs text-slate-400 flex-1">Avg Table Turnaround</span>
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-slate-300">Revenue Leakage — Today</h2>
            <AlertCircle size={15} className="text-red-400" />
          </div>
          <div className="space-y-3">
            <div className="flex items-center gap-3 bg-slate-800/60 rounded-xl px-4 py-3">
              <div className="w-8 h-8 rounded-lg bg-amber-500/15 flex items-center justify-center flex-shrink-0">
                <Ban size={15} className="text-amber-400" />
              </div>
              <div className="flex-1">
                <div className="text-sm text-slate-300 font-medium">Voided Items</div>
                <div className="text-xs text-slate-500">Individual items removed from orders</div>
              </div>
              <div className={`text-xl font-bold ${(summary?.revenuLeakage?.voidedItems ?? 0) > 0 ? 'text-amber-400' : 'text-slate-500'}`}>
            ...
            <div className="flex items-center gap-3 bg-slate-800/60 rounded-xl px-4 py-3">
              ...
              <div className="flex-1">
                <div className="text-sm text-slate-300 font-medium">Cancelled with Value</div>
                <div className="text-xs text-slate-500">Orders cancelled after items were added</div>
              </div>
              <div className={`text-xl font-bold ${(summary?.revenuLeakage?.cancelledWithValue ?? 0) > 0 ? 'text-red-400' : 'text-slate-500'}`}>
      ...
      <div className="card p-0 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-300">Live Orders</h2>
          </div>
              ...
              <tbody>
                {activeOrders.map((order: any) => (
                  <tr key={order.id} className="table-row">
                    <td className="td font-medium text-amber-400">{order.orderNumber}</td>
                    ...
                    <td className="td text-slate-400">{order.itemCount ?? order.items?.length ?? '—'}</td>
