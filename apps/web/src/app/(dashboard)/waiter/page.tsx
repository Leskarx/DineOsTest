  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      <div>
        ...
        <p className="text-sm text-slate-900 dark:text-slate-400">Service Dashboard • {dayjs().format('dddd, D MMMM YYYY')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Link href="/pos" className="group card hover:border-amber-500 transition-all cursor-pointer flex flex-col items-center justify-center p-8 text-center bg-gradient-to-br from-slate-800 to-slate-900">
          ...
          <h3 className="font-bold text-lg text-slate-900 dark:text-white mb-1">Take Order (POS)</h3>
          <p className="text-xs text-slate-900 dark:text-slate-400">Punch in new orders</p>
        </Link>
        ...
        <Link href="/tables" className="group card hover:border-purple-500 transition-all cursor-pointer flex flex-col items-center justify-center p-8 text-center bg-gradient-to-br from-slate-800 to-slate-900">
          ...
          <h3 className="font-bold text-lg text-slate-900 dark:text-white mb-1">Tables</h3>
          <p className="text-xs text-slate-900 dark:text-slate-400">View table statuses & manage guests</p>
        </Link>
        ...
        <Link href="/menu" className="group card hover:border-blue-500 transition-all cursor-pointer flex flex-col items-center justify-center p-8 text-center bg-gradient-to-br from-slate-800 to-slate-900">
          ...
          <h3 className="font-bold text-lg text-slate-900 dark:text-white mb-1">Digital Menu</h3>
          <p className="text-xs text-slate-900 dark:text-slate-400">Browse items & check availability</p>
        </Link>
      </div>
      
      {/* Informational Widget */}
      <div className="mt-8 bg-slate-100/50 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700 rounded-xl p-6 flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
        <div className="w-12 h-12 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 flex items-center justify-center flex-shrink-0">
          <Clock size={24} />
        </div>
        <div>
