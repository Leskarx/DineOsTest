    ...
    <div className="w-full max-w-sm space-y-6">
      ...
      {!validLink ? (
        <div className="card text-center space-y-4">
          ...
          <p className="text-slate-400 text-sm">This link is missing required parameters. Please request a new password reset.</p>
        ...
        <div className="card text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Your password has been updated. Redirecting to login...</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="card space-y-4">
          {error && (
            <div className="flex items-start gap-2 bg-red-900/20 border border-red-700 rounded-xl p-3 text-sm text-red-400">
            ...
            {form.newPassword && (
              <div className="mt-2 space-y-1">
                <div className="flex gap-1">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className={`h-1 flex-1 rounded-full transition-all ${i <= s ? strengthColor : 'bg-slate-700'}`} />
                  ))}
                </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              {form.confirm && (
                <span className={`absolute right-3 top-1/2 -translate-y-1/2 ${form.confirm === form.newPassword ? 'text-emerald-400' : 'text-red-400'}`}>
          ...
          <Link href="/login" className="block text-center text-sm text-slate-400 hover:text-white transition-colors">
            Back to Login
          </Link>
        </form>
      )}
