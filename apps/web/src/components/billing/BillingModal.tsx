...
export function BillingModal({
  ...
  const { branchId, tenantId, branchName } = useAuthStore();
  const {
  ...
  // Use branch name from store, fall back to 'Restaurant'
  const restaurantName = branchName || 'Restaurant';
  ...
  const handlePrint = () => {
    ...
    try {
      printHtml({
        restaurantName,                          // ← branch name from store
        ...
        items: cart.map((i: any) => ({
          ...
          qty:    i.qty,
          rate:   round2(i.price),
          amount: round2(i.price * i.qty),       // ← pre-tax amount
        })),
        subtotal,
            ...
            <div className="p-6 space-y-5">
              ...
              {isDelivery && (
                <div className="rounded-xl border-2 border-amber-400/60 bg-amber-50 dark:bg-amber-950/20 p-4 space-y-3">
                  ...
                  <div className="relative">
                    ...
                    <input
                      className={cn('input pl-8 text-sm', !deliveryPhone.trim() && 'border-red-400 dark:border-red-600')}
              ...
              {!isDelivery && (
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="label">
                      GSTIN <span className="text-slate-400 font-normal">(B2B)</span>
                    </label>
                    <input
