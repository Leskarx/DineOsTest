...
const STATUS_STYLES: Record<string, string> = {
  available: 'border-emerald-600 bg-emerald-100 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-300',
  occupied: 'border-red-600 bg-red-100 dark:bg-red-900/20 text-red-300 opacity-60',
  reserved: 'border-amber-600 bg-amber-100 dark:bg-amber-900/20 text-amber-600 dark:text-amber-300',
  cleaning: 'border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-400 opacity-50',
};
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-300 dark:border-slate-700 w-full max-w-lg shadow-2xl">
        ...
        <div className="p-4 grid grid-cols-4 gap-3 max-h-96 overflow-y-auto">
          {tables?.map((t: any) => (
            ...
            >
              {tableId === t.id && <CheckCircle size={12} className="absolute top-1.5 right-1.5 text-amber-600 dark:text-amber-400" />}
