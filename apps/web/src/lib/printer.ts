...
export function buildReceipt(data: ReceiptData): Uint8Array {
  ...
  // Title
  if (isDelivery) {
    p.alignCenter().boldOn()
      .line(isCOD ? 'DELIVERY ORDER (COD)' : 'DELIVERY ORDER (PREPAID)')
      .boldOff();
  } else if (isTakeaway) {
  ...
  if (isDelivery) {
    ...
    // COD notice on ESC/POS
    if (isCOD) {
      p.divider('-');
      p.boldOn().line('** CASH ON DELIVERY **').boldOff();
      p.line(`  Amount to collect: ${fmt(data.grandTotal)}`);
    }
  }
  ...
  if (isDelivery) {
    ...
    if (isCOD) {
      p.line('Rider will collect payment');
      p.line('on delivery.');
    } else {
...
function buildReceiptHtml(data: ReceiptData): string {
  ...
  const titleText = isDelivery
    ? (isCOD ? 'DELIVERY ORDER — COD' : 'DELIVERY ORDER — PREPAID')
    : isTakeaway
      ? 'TAKEAWAY ORDER'
      : 'TAX INVOICE';

  // Deliver-To section (delivery only)
  ...
  const metaRows = `
    ...
    ${data.tableName && !isDelivery && !isTakeaway
      ? `<tr><td>Table:</td><td class="r">${data.tableName}</td></tr>`
      : ''}
    ${isTakeaway && data.customerName
      ? `<tr><td>Customer:</td><td class="r">${data.customerName}</td></tr>`
      : ''}
    ${isTakeaway && data.customerPhone
      ? `<tr><td>Phone:</td><td class="r">${data.customerPhone}</td></tr>`
      : ''}
    ${data.customerGstin
      ? `<tr><td>Cust GSTIN:</td><td class="r">${data.customerGstin}</td></tr>`
      : ''}
    ${data.cashierName
      ? `<tr><td>Cashier:</td><td class="r">${data.cashierName}</td></tr>`
      : ''}
  `;
  ...
  // Payment rows — COD shows special note, prepaid shows normal methods
  const payRows = isCOD
    ? `<tr>
       ...
       <tr>
         <td colspan="3" class="small" style="padding-top:2px">
    ...
    : data.payments
        .map((p) => `
          <tr>
            <td colspan="2">${p.method.toUpperCase()}</td>
  ...
  const thankYou = isDelivery
    ? isCOD
      ? 'Thank you! Rider will collect payment on delivery.'
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .deliver-row.address span:last-child { font-weight: normal; white-space: pre-wrap; word-break: break-word; }
...
<body>
  ...
  <h2 class="${isDelivery ? (isCOD ? 'delivery' : 'delivery-prepaid') : isTakeaway ? 'takeaway' : ''}">
    ${titleText}
  </h2>
  ...
  <div class="divider"></div>
  ${data.gstin
    ? '<p class="center small" style="margin-top:4px">This is a Computer Generated Invoice</p>'
    : ''}
