...
export class Order {
  ...
  @Column({ name: 'order_type', type: 'enum', enum: OrderType, default: OrderType.DINE_IN })
  type: OrderType;
  ...
  /**
   * Full delivery address — mandatory for delivery orders.
   * Printed on the delivery slip so the rider knows where to go.
   */
   ...
   * 'self'      = restaurant's own rider (default, Option 1)
   * 'porter'    = Porter API (future)
   * 'shadowfax' = Shadowfax API (future)
   * 'zomato'    = Zomato platform order (future)
   * 'swiggy'    = Swiggy platform order (future)
   */
  @Column({ name: 'delivery_provider', type: 'varchar', nullable: true, default: 'self' })
  deliveryProvider: string | null;

  /**
   * When the order was actually delivered to the customer.
   * Set when cashier marks "Delivered".
   */
  ...
  @Column({ name: 'cover_count', type: 'smallint', default: 1 })
  covers: number;

  @Column({ type: 'text', nullable: true })
  notes: string | null;

  @Column({ name: 'kot_printed', type: 'boolean', default: false })
  kotPrinted: boolean;

  @Column({ name: 'bill_printed', type: 'boolean', default: false })
  billPrinted: boolean;
  ...
  @Column({ name: 'is_complimentary', type: 'boolean', default: false })
  isComplimentary: boolean;

  @Column({ name: 'is_sales_return', type: 'boolean', default: false })
  ...
  @Column({ name: 'offline_id', type: 'varchar', nullable: true })
